...
document.addEventListener('DOMContentLoaded', () => {
    ...
    class Recipe {
        ...
        cook() {
            ...
            this.steps.forEach(step => {
                if (step.length === 4) {
                    const [quantity, unit, ingredient, type] = step;
                    const action = type === 'Séchez' ? 'Ajoutez' : 'Versez';
                    recipeString += `<p>${action} ${quantity} ${unit} de ${ingredient} dans le bol.</p>`;
                } else if (step.length === 2) {
                    const [temperature, time] = step;
                    if (temperature !== 0 && time !== 0) {
                        recipeString += `<p>Ensuite, cuire ${time} minutes au four à ${temperature} degrés.</p>`;
                    }
                } else if (typeof step === 'string') {
                    recipeString += `<p>${step}</p>`;
                }
            });

            recipeString += `<p>Bon appétit!</p>`;
            return recipeString;
        }
    }

    const chocolateCookies = new Recipe("Le meilleurs Red Velvet Cake de l'île Britannique", [
    ...
    if (recipeContainer) {
        recipeContainer.innerHTML = chocolateCookies.cook();
    }
